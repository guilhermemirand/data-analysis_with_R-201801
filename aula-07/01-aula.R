...
ggplot(df_geom_probs, aes(x=x, y=y)) +
  geom_col() +
  scale_x_continuous(name = "Chamadas Telefônicas", breaks=0:20) +
  scale_y_continuous(name = "Prob (%)", breaks=seq(from=0, to=100, by=10)) +
  theme_light()
...
seq_head_tails <- rle(sample_head_tails)
seq_head_tails$lengths[!seq_head_tails$values]
tentativas_6_falhas <- round( prop.table( table(seq_head_tails$lengths[!seq_head_tails$values]) ), 2)[6]

paste0(tentativas_6_falhas * 500, ' sequencias de 6 falha')
